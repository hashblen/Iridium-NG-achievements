...
func InitProto() {
	if !config.AutoFindMinimalProto {
		...
		packetIdMap = make(map[uint16]string)
	}
	protoParser.ImportPaths = []string{"./data/", "./out/"}
	LoadProto("PacketHead")
	if config.AutoFindMinimalProto {
		LoadProto("Unk")
	} else {
		LoadProto("Genshin")
...
func parseUnkProto(data []byte) (*dynamic.Message, error) {
	msg := msgMap["Unk"]
	dMsg := dynamic.NewMessage(msg)
